import {
	...
	signSession,
	verifySession
} from '../src/lib/server/auth/auth.js';
...
async function runTests() {
	...
	const spoofedFile = new Uint8Array([
	...
	assert(
		humanSubmit.success === true && humanSubmit.spamCaught === false,
		'Human submission bypasses honeypot'
	);

	const botSubmit = handleSubmission({ name: 'SpamBot', website: 'http://free-clicks.ru' });
	assert(
		botSubmit.success === true && botSubmit.spamCaught === true,
		'Bot submission caught by honeypot and silently swallowed'
	);
