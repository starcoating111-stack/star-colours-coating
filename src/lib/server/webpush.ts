...
async function hkdf(salt: ArrayBuffer, ikm: ArrayBuffer, info: ArrayBuffer, bits: number): Promise<ArrayBuffer> {
...
async function vapidJwt(audience: string, subject: string, privateJwk: JsonWebKey): Promise<string> {
  ...
  const body = bufToB64u(strBuf(JSON.stringify({
    aud: audience,
...
async function encrypt(sub: PushSub, plaintext: string): Promise<ArrayBuffer> {
  ...
  const shared = await crypto.subtle.deriveBits({ name: 'ECDH', public: recvKey }, ephem.privateKey, 256);
