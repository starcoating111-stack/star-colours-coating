<script lang="ts">
	...
	const waNum = $derived(
		siteConfig.showFloatingWhatsApp && whatsappNumber ? whatsappNumber.replace(/[^\d]/g, '') : ''
	);

	const waUrl = $derived(
		waNum ? `https://wa.me/${waNum}?text=${encodeURIComponent(whatsappDefaultMessage)}` : ''
	);
</script>
...
{#if waUrl}
	...
	>
		...
		<span class="hidden sm:inline font-outfit tracking-wide">WhatsApp</span>
	</a>
{/if}
