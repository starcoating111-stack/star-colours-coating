<script lang="ts">
  ...
  const defaultTestimonials = [
    ...
    },
    {
      ...
      review: "The finishing work was exceptionally detailed and durable. Highly recommend their PU coating solutions for luxury homes.",
      rating: 5,
      photoUrl: ""
    }
  ];
...
<section id="testimonials" class="py-20 bg-[#c2bebe] px-6 text-zinc-900 border-t border-zinc-900/10 selection:bg-zinc-800 selection:text-white">
  <div class="max-w-4xl mx-auto relative flex flex-col items-center">
    
    {#if list.length === 0}
      <div class="text-zinc-655 text-sm py-12">
    ...
    {:else}
      ...
      <div class="w-full flex flex-col items-center text-center space-y-6 select-none relative">
        ...
        <!-- Review Text -->
        <p class="text-base sm:text-lg md:text-xl text-zinc-800 italic max-w-2xl leading-relaxed font-sans px-4">
          {active.review}
        </p>
        ...
        <div class="flex items-center justify-center gap-6 pt-4 w-full max-w-md mx-auto">
          ...
          <div class="relative">
            {#if active.photoUrl}
              <img
                src={active.photoUrl.includes('/') || active.photoUrl.includes('.') ? `/images/${active.photoUrl.split('/').pop()}` : `/images/${active.photoUrl}`}
                alt={active.name}
                class="w-16 h-16 rounded-full object-cover bg-white/50 border border-white shadow-md"
              />
            {:else}
              <div class="w-16 h-16 rounded-full bg-zinc-400 text-zinc-900 border border-white shadow-md flex items-center justify-center font-bold text-lg font-outfit">
          ...
          <div class="flex items-center gap-1.5 pt-4">
            {#each list as _, idx}
              <button
                ...
                class="h-1.5 rounded-full transition-all {currentIndex === idx ? 'w-6 bg-zinc-800' : 'w-1.5 bg-zinc-500 hover:bg-zinc-700'}"
