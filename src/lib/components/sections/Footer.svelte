...
<footer
	class="bg-[#180f0c] border-t border-zinc-900/60 text-zinc-450 py-16 selection:bg-zinc-800 selection:text-white font-sans"
>
	<div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
		...
		<div class="space-y-4">
			...
			<ul class="space-y-2 text-xs text-zinc-500">
				{#if settings.phone}
					<li>
						Phone: <a href="tel:{settings.phone}" class="hover:text-zinc-300">{settings.phone}</a>
					</li>
				{/if}
				{#if settings.email}
					<li>
						Email: <a href="mailto:{settings.email}" class="hover:text-zinc-300">{settings.email}</a
						>
					</li>
				{/if}
				{#if settings.address}
	...
	<div
		class="max-w-7xl mx-auto px-6 border-t border-zinc-900/40 mt-16 pt-8 flex flex-col md:flex-row items-center justify-between gap-4 text-[10px] text-zinc-650"
	>
		<span
			>&copy; {currentYear}
			{settings.companyName || 'Star Colours Coating'}. All rights reserved.</span
		>
		<span
			>Designed & Developed by <a
				href="https://codenxtlab.tech/"
				class="hover:text-zinc-500 underline transition-colors">CodeNXT Lab</a
			></span
		>
	</div>
</footer>
