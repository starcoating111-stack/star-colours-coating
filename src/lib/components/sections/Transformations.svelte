<script lang="ts">
	...
	const currentCase = $derived(transformations[currentIndex]);

	function handlePrev() {
		currentIndex = (currentIndex - 1 + transformations.length) % transformations.length;
				...
				>
					<img
						src={activeTab === 'before'
							? `/images/${currentCase.beforeImageUrl}`
							: `/images/${currentCase.afterImageUrl}`}
