...
<main class="min-h-screen flex items-center justify-center bg-zinc-950 text-zinc-100 p-4 selection:bg-zinc-800 selection:text-white relative overflow-hidden">
  <!-- Decorative background elements -->
  <div class="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-zinc-900/40 blur-3xl -z-10 animate-pulse duration-10000"></div>
  <div class="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-zinc-900/30 blur-3xl -z-10 animate-pulse duration-7000"></div>

  <div class="w-full max-w-md bg-zinc-900/60 border border-zinc-800 backdrop-blur-xl rounded-2xl p-8 shadow-2xl relative">
    <div class="flex flex-col items-center mb-8">
      <div class="w-12 h-12 rounded-xl bg-gradient-to-tr from-zinc-700 to-zinc-900 border border-zinc-700 flex items-center justify-center mb-4 shadow-lg">
    ...
    {#if form?.message}
      <div class="mb-6 p-4 rounded-xl bg-red-950/40 border border-red-900/50 text-red-400 text-sm flex items-start gap-3">
    ...
    <form method="POST" use:enhance={() => {
      ...
      <div>
        <label for="username" class="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Username</label>
        <input
          type="text"
          name="username"
          id="username"
