...
<div>
  ...
  <div class="mb-8">
    ...
    <p class="text-zinc-400 text-sm mt-1">Review contact form submissions, inquiries, and spam logs.</p>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
    ...
    <div class="lg:col-span-1 space-y-4">
      <!-- Status Filter Tabs -->
      <div class="flex bg-zinc-950 border border-zinc-900 p-1.5 rounded-xl text-xs gap-1 font-semibold text-zinc-400">
        <button
          onclick={() => filter = 'all'}
          class="flex-1 py-2 text-center rounded-lg transition-colors {filter === 'all' ? 'bg-zinc-900 text-white' : 'hover:text-zinc-200'}"
        >
          All
        </button>
        <button
          onclick={() => filter = 'new'}
          class="flex-1 py-2 text-center rounded-lg transition-colors {filter === 'new' ? 'bg-zinc-900 text-white' : 'hover:text-zinc-200'}"
        >
          New
        </button>
        <button
          onclick={() => filter = 'read'}
          class="flex-1 py-2 text-center rounded-lg transition-colors {filter === 'read' ? 'bg-zinc-900 text-white' : 'hover:text-zinc-200'}"
        >
          Read
        </button>
        <button
          onclick={() => filter = 'archived'}
          class="flex-1 py-2 text-center rounded-lg transition-colors {filter === 'archived' ? 'bg-zinc-900 text-white' : 'hover:text-zinc-200'}"
        >
          Archived
        </button>
      </div>
      ...
      <div class="space-y-3 max-h-[500px] overflow-y-auto pr-1">
        {#if filteredMessages.length === 0}
          <div class="border border-dashed border-zinc-900 rounded-xl p-8 text-center text-zinc-600 text-xs">
        ...
        {:else}
          {#each filteredMessages as msg}
            ...
            <button
              ...
              class="w-full text-left p-4 rounded-xl border transition-all flex flex-col gap-2 relative bg-zinc-900/10 hover:bg-zinc-900/30 {isSelected ? 'border-white' : 'border-zinc-900'}"
            >
              ...
              <div class="flex items-center justify-between mt-1 w-full">
                <!-- Status indicator -->
                {#if msg.status === 'new'}
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-red-950/40 border border-red-900/40 text-red-400">
                    New
                  </span>
                {:else if msg.status === 'read'}
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-zinc-900 text-zinc-400">
                    Read
                  </span>
                {:else}
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-zinc-900/50 border border-zinc-900 text-zinc-500">
                    Archived
                  </span>
                {/if}
              </div>
    ...
    <div class="lg:col-span-2">
      {#if !selectedMessage}
        <div class="border border-dashed border-zinc-900 rounded-2xl h-[420px] flex flex-col items-center justify-center text-zinc-500 p-8">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-zinc-600 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0l-3.586-3.586a2 2 0 00-2.828 0L16 14m-8.657 0l-3-3m0 0l3-3m-3 3h12.75"/>
          </svg>
      ...
      {:else}
        <div class="bg-zinc-900/10 border border-zinc-900 rounded-2xl p-6 space-y-6 backdrop-blur">
          ...
          <div class="flex items-start justify-between border-b border-zinc-900 pb-5 gap-4">
            <div>
              <h3 class="font-bold text-lg text-white mb-2">{selectedMessage.subject || '(No Subject)'}</h3>
          ...
          <!-- Message Body -->
          <div class="text-zinc-200 text-sm leading-relaxed whitespace-pre-wrap min-h-[150px] bg-zinc-950 p-4 rounded-xl border border-zinc-900">
