<script lang="ts">
  ...
  let { data } = $props<{
    data: {
      ...
      recentMessages: Array<{ id: number; name: string; email: string; subject: string | null; message: string; status: string; createdAt: string | null }>;
      recentProjects: Array<{ id: number; title: string; slug: string; category: string; location: string }>;
    }
  }>();
  ...
  async function enablePush() {
    ...
    try {
      ...
      if (permission !== 'granted') { pushStatus = 'denied'; return; }
...
<div>
  ...
  <div class="mb-8">
    ...
    <p class="text-zinc-400 text-sm mt-1">Real-time statistics and recent client engagement activity.</p>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
    ...
    <div class="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-6 backdrop-blur">
      <div class="flex items-center justify-between mb-4">
        <span class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Total Projects</span>
    ...
    <div class="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-6 backdrop-blur">
      <div class="flex items-center justify-between mb-4">
        <span class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Gallery Assets</span>
    ...
    <div class="bg-zinc-900/40 border border-zinc-900 rounded-2xl p-6 backdrop-blur">
      <div class="flex items-center justify-between mb-4">
        <span class="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Inbound Messages</span>
        {#if newMessages > 0}
          <span class="text-xs px-2.5 py-0.5 rounded-full bg-red-950/40 border border-red-900/40 text-red-400 font-medium">{newMessages} unread</span>
        {:else}
  ...
  {#if pushStatus !== 'unsupported'}
    <div class="mb-10 bg-zinc-900/20 border border-zinc-900 rounded-2xl p-5 flex items-center justify-between gap-6">
      <div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    ...
    <div class="bg-zinc-900/20 border border-zinc-900 rounded-2xl p-6">
      <div class="flex items-center justify-between mb-6">
        ...
        <a href="/admin/messages" class="text-xs text-zinc-400 hover:text-white transition-colors">View All</a>
      </div>

      {#if data.recentMessages.length === 0}
        <div class="h-48 border border-dashed border-zinc-850 rounded-xl flex items-center justify-center text-zinc-500 text-sm">
          No message submissions yet
        </div>
      {:else}
        <div class="space-y-4">
          {#each data.recentMessages as msg}
            <div class="p-4 rounded-xl bg-zinc-900/40 border border-zinc-850 flex items-start justify-between gap-4">
              <div class="min-w-0">
    ...
    <div class="bg-zinc-900/20 border border-zinc-900 rounded-2xl p-6">
      <div class="flex items-center justify-between mb-6">
        ...
        <a href="/admin/projects" class="text-xs text-zinc-400 hover:text-white transition-colors">Manage Portfolio</a>
      </div>
      ...
      {:else}
        <div class="space-y-4">
          {#each data.recentProjects as proj}
            <div class="p-4 rounded-xl bg-zinc-900/40 border border-zinc-850 flex items-center justify-between">
              <div>
