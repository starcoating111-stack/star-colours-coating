...
export const actions: Actions = {
  updateSettings: async ({ request, platform }) => {
    ...
    if (whatsappDefaultMessage && whatsappDefaultMessage.length > 200) {
      return fail(400, { settingsError: 'Default WhatsApp message must be 200 characters or fewer.' });
    }
    ...
    if (normalizedWhatsApp) {
      if (normalizedWhatsApp.startsWith('0')) {
        return fail(400, { settingsError: 'WhatsApp number must start with a country code, not 0. Example: 447911123456 for a UK +44 number.' });
      }
      if (normalizedWhatsApp.length < 7 || normalizedWhatsApp.length > 15) {
        return fail(400, { settingsError: 'WhatsApp number must be 7–15 digits including country code (no +, spaces, or dashes).' });
      }
    }
  ...
  updatePassword: async ({ request, cookies, platform }) => {
    ...
    try {
      ...
      return { passwordSuccess: 'Password changed successfully! All other active sessions have been signed out.' };
    } catch (err: any) {
