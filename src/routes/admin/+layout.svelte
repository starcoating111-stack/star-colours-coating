...
{:else}
	<div
		class="min-h-screen flex bg-zinc-950 text-zinc-100 selection:bg-zinc-800 selection:text-white font-sans"
	>
		<!-- Sidebar -->
		<aside
			class="w-64 border-r border-zinc-900 bg-zinc-900/30 backdrop-blur flex flex-col flex-shrink-0"
		>
			<!-- Sidebar Header -->
			...
			<nav class="flex-1 px-4 py-6 space-y-1">
				{#each navigation as item}
					...
					<a
						href={item.path}
						class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors {isActive
		...
		<div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
			<!-- Top Header -->
			<header
				class="h-16 border-b border-zinc-900 bg-zinc-900/10 backdrop-blur px-8 flex items-center justify-between"
			>
				<div>
