...
export const actions: Actions = {
	create: async ({ request, platform }) => {
		...
		const videoUrlLines = videoUrlsRaw
			.split('\n')
			.map((l) => l.trim())
			.filter(Boolean);
		for (const url of videoUrlLines) {
			if (!parseVideoUrl(url)) {
				return fail(400, {
					error: `Unrecognized video URL: "${url}". Paste a YouTube, Vimeo, or Loom link.`
				});
			}
		}
	...
	update: async ({ request, platform }) => {
		...
		try {
			...
			if (deleteMediaIdsStr) {
				...
				for (const mediaId of deleteIds) {
					const [record] = await db
						.select()
						.from(projectImages)
						.where(eq(projectImages.id, mediaId));
					if (record) {
			...
			for (const file of galleryFiles) {
				if (file && file.size > 0) {
					...
					await db
						...
						.values({ projectId: id, mediaType: 'image', imageUrl: imgKey, sortOrder: sortIdx });
					sortIdx += 10;
				}
			}
			...
			for (const url of videoUrlLines) {
				await db
					.insert(projectImages)
					.values({ projectId: id, mediaType: 'video', videoUrl: url, sortOrder: sortIdx });
				sortIdx += 10;
			}
