	...
	<div class="flex items-center justify-between mb-8">
		<div>
			...
			<p class="text-zinc-400 text-sm mt-1">
				Upload and manage standalone gallery assets, categorized for the public site.
			</p>
		</div>
		<button
			onclick={openUpload}
			...
			{#each data.gallery as item}
				...
				>
					...
					<div class="aspect-[4/3] w-full bg-zinc-950 relative overflow-hidden">
						...
						{#if item.category}
							<span
								class="absolute top-3 left-3 px-2 py-0.5 rounded-full text-[10px] font-bold bg-zinc-950/70 border border-zinc-800 text-zinc-300 backdrop-blur"
							>
								{item.category}
							</span>
						{/if}
