    ...
    {@html `<script type="application/ld+json">
      {
        "@context": "https://schema.org",
        "@type": "ItemList",
        "itemListElement": [${data.media.filter(m => m.mediaType === 'video' && m.embedUrl).map((m, i) => `
          {
            "@type": "VideoObject",
            "position": ${i + 1},
            "name": "${data.project.title}",
...
<main class="bg-zinc-950 text-zinc-100 selection:bg-zinc-800 selection:text-white py-16 px-6">
  <div class="max-w-6xl mx-auto space-y-16">
    ...
    <div class="aspect-[21/9] w-full bg-zinc-950 overflow-hidden border border-zinc-900 rounded-3xl">
      <img src="/images/{data.project.coverImageUrl}" alt={data.project.title} class="w-full h-full object-cover" />
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {#each data.media as item}
            ...
            {:else if item.embedUrl}
              ...
              <div class="{item.isVertical ? 'aspect-[9/16]' : 'aspect-[16/9]'} w-full rounded-2xl overflow-hidden bg-zinc-950 border border-zinc-900">
                ...
                {:else}
                  ...
                  >
                    {#if item.thumbnailUrl}
                      <img src={item.thumbnailUrl} alt="Video thumbnail" class="w-full h-full object-cover group-hover:scale-102 transition-transform duration-500" />
                    {:else}
                      <div class="w-full h-full bg-zinc-900"></div>
                    {/if}
                    <!-- Play overlay -->
                    <div class="absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/30 transition-colors">
                      <div class="w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center group-hover:scale-110 transition-transform">
