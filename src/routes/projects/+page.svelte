...
<svelte:head>
	...
	<meta
		name="description"
		content="Browse our complete portfolio of case studies detailing structural layouts, web engineering projects, and brand design systems."
	/>
...
<main class="py-20 bg-brand-dark px-6 selection:bg-zinc-800 selection:text-white">
	<div class="max-w-7xl mx-auto space-y-16 animate-fade-in-up">
		...
		{#if data.projects.length === 0}
			<div
				class="border border-dashed border-zinc-900 rounded-2xl p-12 text-center text-zinc-650 text-sm"
			>
		...
		{:else}
			<div class="grid grid-cols-1 md:grid-cols-2 gap-8">
				{#each data.projects as project}
					...
					>
						<div
							class="aspect-[16/10] w-full bg-zinc-950 overflow-hidden relative border-b border-zinc-900/40"
						>
							<img
							...
							/>
							<span
								class="absolute top-4 left-4 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-zinc-950/80 border border-zinc-850 text-zinc-300 backdrop-blur font-outfit uppercase"
							>
								{project.category}
							</span>
						</div>
						<div class="p-6 md:p-8 flex items-center justify-between gap-4">
							<div>
								<h3
									class="font-bold text-lg md:text-xl text-white group-hover:text-brand-accent transition-colors font-outfit uppercase leading-tight"
								>
									{project.title}
								</h3>
							...
							<div
								class="w-10 h-10 rounded-full border border-zinc-800 flex items-center justify-center text-zinc-400 group-hover:text-white group-hover:border-brand-accent transition-colors"
							>
								&rarr;
							</div>
						</div>
