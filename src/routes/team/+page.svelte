...
<main class="py-20 bg-brand-dark px-6 selection:bg-zinc-800 selection:text-white">
  <div class="max-w-7xl mx-auto space-y-16 animate-fade-in-up">
    ...
    <div class="text-center space-y-4 max-w-xl mx-auto">
      <span class="text-[10px] font-bold text-brand-accent uppercase tracking-widest font-outfit">Our People</span>
    ...
    {#if data.teamMembers.length === 0}
      <div class="border border-dashed border-zinc-900 rounded-2xl p-12 text-center text-zinc-500 text-sm font-sans">
        Team information coming soon.
      </div>
    {:else}
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {#each data.teamMembers as member}
          <div class="bg-brand-card border border-zinc-900/50 rounded-3xl p-6 flex flex-col items-center text-center space-y-4 hover:border-brand-accent/20 transition-all duration-300">
            <!-- Photo -->
            {#if member.photoUrl}
              <img
