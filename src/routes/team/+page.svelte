...
<svelte:head>
  ...
  <meta name="description" content="Meet the dedicated professionals on our team — the people who bring expertise, passion, and craft to every project." />
...
<main class="py-20 bg-zinc-950 px-6 selection:bg-zinc-800 selection:text-white">
  <div class="max-w-7xl mx-auto space-y-16">
    ...
    <div class="text-center space-y-4 max-w-xl mx-auto">
      ...
      <h1 class="text-4xl font-extrabold text-white tracking-tight">Meet the Team</h1>
    ...
    {:else}
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {#each data.teamMembers as member}
          <div class="bg-zinc-900/10 border border-zinc-900 rounded-2xl p-6 flex flex-col items-center text-center space-y-4 backdrop-blur">
            <!-- Photo -->
            {#if member.photoUrl}
              <img
              ...
              />
            {:else}
              <div class="w-20 h-20 rounded-full bg-zinc-900 border-2 border-zinc-800 flex items-center justify-center font-bold text-zinc-400 text-xl">
            ...
            <!-- Name & Role -->
            <div>
              <h2 class="font-bold text-white text-base">{member.name}</h2>
              <p class="text-zinc-500 text-xs mt-0.5">{member.designation}</p>
            </div>

            <!-- Bio -->
            {#if member.bio}
              <p class="text-zinc-400 text-xs leading-relaxed">{member.bio}</p>
            {/if}
              ...
              <div class="flex items-center gap-3 pt-2">
                {#if member.email}
                  <a
                    ...
                    class="text-zinc-500 hover:text-white transition-colors text-xs underline underline-offset-2"
