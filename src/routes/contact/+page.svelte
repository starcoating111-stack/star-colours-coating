...
<svelte:head>
	...
	<meta
		name="description"
		content="Reach out via WhatsApp for a quick reply, or fill out the inquiry form."
	/>
	...
	<div class="max-w-6xl mx-auto space-y-12">
		...
		<div class="text-center space-y-4">
			...
			<p class="text-zinc-400 text-sm max-w-md mx-auto leading-relaxed">
				{#if waNum}
					Chat with us instantly on WhatsApp, or leave a written note below — whatever works best
					for you.
				{:else}
		...
		{#if form?.success}
			...
			<div
				class="max-w-xl mx-auto bg-zinc-900/10 border border-zinc-900 rounded-3xl p-8 text-center space-y-4"
			>
				<div
					class="w-12 h-12 rounded-full bg-emerald-950/40 border border-emerald-900/40 text-emerald-400 flex items-center justify-center mx-auto text-xl font-bold"
				>
					✓
				</div>
				<h3 class="font-bold text-white text-lg">Submission Received</h3>
				<p class="text-xs text-zinc-400 leading-relaxed">
					{form.message} We have dispatched a confirmation email to your address.
				</p>
			</div>
		{:else if waNum}
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
				...
				<div
					class="bg-zinc-900/20 border border-zinc-900 rounded-3xl p-8 flex flex-col gap-8 backdrop-blur"
				>
					<!-- Icon + heading -->
					<div class="space-y-4">
						<div
							class="w-14 h-14 rounded-2xl bg-[#25D366]/10 border border-[#25D366]/20 flex items-center justify-center"
						>
							<svg
								class="w-7 h-7 text-[#25D366]"
								viewBox="0 0 24 24"
								fill="currentColor"
								aria-hidden="true"
							>
						...
						<div>
							...
							<p class="text-zinc-400 text-sm mt-2 leading-relaxed">
								Get a reply in minutes. No forms, no waiting — just a direct conversation with our
								team.
							</p>
						</div>
					</div>
					...
					<ul class="space-y-3">
						{#each ['Instant responses during business hours', 'Share photos, documents, or voice notes', 'Pick up the conversation anytime'] as perk}
							<li class="flex items-center gap-3 text-sm text-zinc-300">
								<span
									class="w-5 h-5 rounded-full bg-[#25D366]/10 border border-[#25D366]/20 flex items-center justify-center flex-shrink-0"
								>
					...
					<div class="mt-auto">
						...
						{#if name.trim() || message.trim()}
							<p class="text-[10px] text-zinc-500 text-center mt-3">
								Your message will be pre-filled when WhatsApp opens.
							</p>
						{/if}
					</div>
				</div>
				...
				<div class="bg-zinc-900/10 border border-zinc-900 rounded-3xl p-8 backdrop-blur">
					...
					{#if form?.error}
						<div
							class="mb-6 p-4 rounded-xl bg-red-950/20 border border-red-900/30 text-red-400 text-xs"
						>
							{form.error}
						</div>
					{/if}
					...
					>
						...
						<div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
							<div>
								<label
									for="name"
									class="block text-[10px] font-semibold text-zinc-400 uppercase tracking-wider mb-2"
									>Your Name *</label
								>
								<input
									type="text"
		...
		{:else}
			...
			<div
				class="max-w-xl mx-auto bg-zinc-900/10 border border-zinc-900 rounded-3xl p-8 backdrop-blur"
			>
				{#if form?.error}
					<div
						class="mb-6 p-4 rounded-xl bg-red-950/20 border border-red-900/30 text-red-400 text-xs sm:text-sm"
					>
						{form.error}
					</div>
				{/if}
				...
				>
					...
					<div>
						<label
							for="message-f"
							class="block text-[10px] font-semibold text-zinc-400 uppercase tracking-wider mb-2"
							>Brief Project Scope * (Min 10 chars)</label
						>
						<textarea
							id="message-f"
							name="message"
