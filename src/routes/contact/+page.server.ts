...
import { sendEmailViaZeptoMail, getClientEmailTemplate, getOwnerEmailTemplate } from '$lib/server/email/zeptomail';
...
export const actions: Actions = {
  default: async (event) => {
    ...
    if (!validation.success) {
      const errorMsg = validation.error.issues[0]?.message || 'Invalid form inputs. Please check your data.';
        ...
        if (apiKey && bounceEmail) {
          try {
            ...
            await sendEmailViaZeptoMail(
              apiKey, bounceEmail, companyName,
            ...
            await sendEmailViaZeptoMail(
              apiKey, bounceEmail, companyName,
              ownerEmail, 'Site Administrator',
              `New Inquiry: ${validatedData.subject || 'No Subject'}`,
              getOwnerEmailTemplate(companyName, validatedData.name, validatedData.email, validatedData.phone ?? null, validatedData.subject ?? null, validatedData.message)
            );
          } catch (emailErr) {
          ...
          try {
            ...
            await Promise.allSettled(subs.map(async (row) => {
              try {
                ...
                await sendWebPush(
                  sub,
                  { title: 'New Inquiry', body: `From ${validatedData.name}`, url: '/admin/messages' },
                  vapidPublicKey, vapidPrivateJwk, vapidSubject
                );
              } catch (err: any) {
                if (err?.status === 410) {
