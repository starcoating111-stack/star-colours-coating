...
<svelte:head>
	...
	<meta
		name="description"
		content="Explore our high-resolution image gallery showcasing modern designs, web developments, and structural works."
	/>
			...
			<div class="flex flex-wrap items-center justify-center gap-2 max-w-lg mx-auto">
				{#each categories as category}
					<button
						...
						class="text-xs font-semibold px-4 py-2 rounded-xl border transition-colors {activeCategory ===
						category
							? 'bg-white border-white text-zinc-950'
							: 'bg-zinc-900/30 border-zinc-900 text-zinc-400 hover:text-white hover:border-zinc-800'}"
					>
						{category}
					</button>
				{/each}
		...
		{:else}
			<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
				{#each filteredItems as item}
					<div
						class="group bg-zinc-900/10 border border-zinc-900 rounded-2xl overflow-hidden backdrop-blur flex flex-col relative"
					>
						<div
							class="aspect-[4/3] w-full bg-zinc-950 overflow-hidden relative border-b border-zinc-900"
						>
							<img
							...
							{#if item.category}
								<span
									class="absolute top-3 left-3 px-2 py-0.5 rounded-full text-[9px] font-bold bg-zinc-950/70 border border-zinc-850 text-zinc-300 backdrop-blur"
								>
									{item.category}
								</span>
							{/if}
