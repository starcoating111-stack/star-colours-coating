...
```
*Note: `.dev.vars` is ignored by Git and used exclusively for Wrangler local emulation.*
...
npm run db:seed
```
*Console output will display the generated admin credentials:*
*   **Username**: `admin@codenxt.local`
*   **Password**: *(Dynamic 16-character alphanumeric password logged to terminal)*
...
```
*(Tip: You can copy-paste the pre-salted/pre-hashed scrypt outputs logged during your local `npm run db:seed` run to seed the production database quickly).*
